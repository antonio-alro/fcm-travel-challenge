  ...
  describe '#perform' do
    context 'when the input is not valid' do
      it 'raises an error' do
        fake_input = OpenStruct.new(base_city: 'SVQ', filename: 'fake_input.txt', invalid?: true, invalid_input_error: InvalidInputError)

        use_case = use_case_klass.new(
          get_segments_from_input_service: self.class::FakeGetSegmentsFromInputService.with_segments([])
        )

        expect { use_case.perform(input: fake_input) }.to raise_error(InvalidInputError)
      end
    end

    context 'when the input is valid' do
      it 'calls the expected services and return the list of trips' do
        fake_input = OpenStruct.new(base_city: 'SVQ', filename: 'fake_input.txt', invalid?: false)
